...
!(function ($) {
    ...
    var Datepicker = function (element, options) {
        ...
        this.language =
            this.language in dates
                ? this.language
                : this.language.split("-")[0]; //Check if "de-DE" style date is available, if not language should fallback to 2 letter code eg "de"
        ...
        this.picker = $(
            DPGlobal.template(this.leftArrow, this.rightArrow, this.closeIcon),
        )
        ...
        this.setDatesDisabled(
            options.datesDisabled || this.element.data("dates-disabled"),
        );

        if (this.initialDate != null) {
            this.date = this.viewDate = DPGlobal.parseDate(
                this.initialDate,
                this.format,
                this.language,
            );
    ...
    Datepicker.prototype = {
                    ...
                    this._events = [
                        [
                            this.element,
                            {
                                ...
                                click: this.element.attr("readonly")
                                    ? $.proxy(this.show, this)
                                    : function () {},
                            },
                        ],
        ...
        place: function () {
            ...
            var height =
                textbox.outerHeight() + parseInt(textbox.css("margin-top"));
            var width =
                textbox.outerWidth() + parseInt(textbox.css("margin-left"));
            var fullOffsetTop = offset.top + height;
            var offsetLeft = offset.left;
            this.picker.removeClass("datepicker-top datepicker-bottom");
            // can we show it on top?
            var canShowTop =
                $(window).scrollTop() < offset.top - this.picker.outerHeight();
            var canShowBottom =
                fullOffsetTop + this.picker.outerHeight() <
        ...
        fill: function () {
            ...
            var d = new Date(this.viewDate.valueOf()),
                ...
                today = new Date(),
                titleFormat =
                    dates[this.language].titleFormat || dates["en"].titleFormat;
            ...
            this.picker
                .find(".datepicker-hours thead th:eq(1)")
                .text(
                    dayMonth +
                        " " +
            ...
            this.picker
                .find(".datepicker-minutes thead th:eq(1)")
                .text(
                    dayMonth +
                        " " +
                        dates[this.language].months[month] +
            ...
            for (var i = 0; i < 24; i++) {
                ...
                if (this.nonMilitaryTime) {
                    ...
                    html.push(
                        '<span class="hour nonMilitaryTime' +
                            clsName +
                            '">' +
                            tts +
                            ":00 " +
                    ...
                    html.push(
                        '<span class="hour' + clsName + '">' + i + ":00</span>",
                    );
                }
            }
            ...
            for (var i = 0; i < 60; i += this.minuteStep) {
                ...
                if (this.nonMilitaryTime) {
                    ...
                    html.push(
                        '<span class="minute nonMilitaryTime' +
                            clsName +
                            '">' +
                            tts +
                            ":" +
                            ...
                            if (target.is(".month")) {
                                ...
                                if (this.nonMilitaryTime) {
                                    if (target.text().indexOf("AM") >= 0) {
                                        hours =
                                            parseInt(target.text(), 10) || 0;
                                    } else {
                                        hours =
                                            parseInt(target.text(), 10) + 12 ||
                                            0;
                                    }
                                } else {
    ...
    DPGlobal.template = function (leftArrow, rightArrow, closeIcon) {
        return (
            ...
            "</table>" +
            "</div>" +
            '<a class="button datepicker-close tiny alert right" style="width:auto;">' +
            closeIcon +
            "</a>" +
            "</div>"
        );
